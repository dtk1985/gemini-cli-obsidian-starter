...
```bash
cat "Daily/$(date -d "yesterday" +%Y-%m-%d). Morning Check-in.md" 2>/dev/null
```
*(Note: adapted `date` command for Linux/GNU)*
...
`.gemini/skills-src/review/assets/morning-checkin.md`
